...
class NotificationData {
  ...
  NotificationData.fromJson(dynamic json) {
    ...
    var id=json['sender_user_id'];
    if(id is int){
      _senderUserId =json['sender_user_id'];
    }else{
      _senderUserId =int.parse(json['sender_user_id']);
    }
    var receiveId=json['received_user_id'];
    if(receiveId is int){
      _receivedUserId =json['received_user_id'];
    }else{
      _receivedUserId =int.parse(json['received_user_id']);
    }

    var notiType=json['notification_type'];
    if(notiType is int){
      _notificationType =json['notification_type'];
    }else{
      _notificationType =int.parse(json['notification_type']);
    }
    var itemId=json['item_id'];
    if(itemId is int){
      _itemId =json['item_id'];
    }else{
      _itemId =int.parse(json['item_id']);
    }
...
class SenderUser {
  ...
  SenderUser.fromJson(dynamic json) {
    ...
    if(platform is int){
      ...
      _platform =int.parse(json['platform']);
    }
    _deviceToken = json['device_token'];
    var verify=json['is_verify'];
    if(verify is int){
      _isVerify =json['is_verify'];
    }else{
      _isVerify =int.parse(json['is_verify']);
    }

    var total_received=json['total_received'];
    if(total_received is int){
      _totalReceived =json['total_received'];
    }else{
      _totalReceived =int.parse(json['total_received']);
    }
    var total_send=json['total_send'];
    if(total_send is int){
      _totalSend =json['total_send'];
    }else{
      _totalSend =int.parse(json['total_send']);
    }
    var my_wallet=json['my_wallet'];
    if(my_wallet is int){
      _myWallet =json['my_wallet'];
    }else{
      _myWallet =int.parse(json['my_wallet']);
    }

    var spen_in_app=json['spen_in_app'];
    if(spen_in_app is int){
    ...
    if(check_in is int){
      ...
      _checkIn =int.parse(json['check_in']);
    }
    var upload_video=json['upload_video'];
    if(upload_video is int){
      _uploadVideo =json['upload_video'];
    }else{
      _uploadVideo =int.parse(json['upload_video']);
    }

    var from_fans=json['from_fans'];
    if(from_fans is int){
      _fromFans =json['from_fans'];
    }else{
      _fromFans =int.parse(json['from_fans']);
    }
    var purchased=json['purchased'];
    if(purchased is int){
      _purchased =json['purchased'];
    }else{
      _purchased =int.parse(json['purchased']);
    }
    ...
    var freez_or_not=json['freez_or_not'];
    if(freez_or_not is int){
      _freezOrNot =json['freez_or_not'];
    }else{
      _freezOrNot =int.parse(json['freez_or_not']);
    }
